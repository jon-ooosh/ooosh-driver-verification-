...
// Enhanced UX with spam notice, loading spinner, start again option

import React, { useState, useEffect } from 'react';
import { AlertCircle, CheckCircle, Upload, FileText, Shield, Mail, XCircle, Phone, Camera } from 'lucide-react';
...
const DriverVerificationApp = () => {
  ...
  const validateJobAndFetchDetails = async (jobId) => {
    ...
    try {
      ...
      if (!response.ok || !result.valid) {
        setError(result.message || 'This hire is no longer available or has expired.');
  ...
  const processDVLACheck = async (file) => {
    ...
    try {
      ...
      // Use OCR to extract DVLA check data
  ...
  const renderLanding = () => (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg p-6">
      <div className="text-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Ooosh Tours</h1>
        <h2 className="text-lg font-medium text-gray-700 mt-2">Hire Agreement - Proposal for Insurance</h2>
      </div>
  ...
  const renderEmailEntry = () => (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg p-6">
      <div className="text-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Ooosh Tours</h1>
        <h2 className="text-lg font-medium text-gray-700 mt-1">Hire Agreement - Proposal for Insurance</h2>
        {jobDetails && (
          <p className="text-sm text-gray-600 mt-2">Job: {jobDetails.jobName || jobId}</p>
        )}
      </div>
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
        >
          {loading ? (
            <>
  ...
  // ENHANCED EMAIL VERIFICATION with spam notice, loading spinner, and start again
  const renderEmailVerification = () => (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg p-6">
      <div className="text-center mb-6">
        ...
        <p className="text-sm font-medium text-gray-900 break-words">{driverEmail}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* Enhanced Verify Button with Loading Spinner */}
        <button
        ...
        <button
          onClick={sendVerificationEmail}
          disabled={loading}
          className="w-full text-purple-600 hover:text-purple-700 text-sm disabled:opacity-50 disabled:cursor-not-allowed py-2"
        >
      ...
      {/* Test Email Indicator (if applicable) */}
    ...
    return (
      <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="space-y-3">
          ...
          {/* Start Again Option for Driver Status */}
          <button
            onClick={startAgain}
